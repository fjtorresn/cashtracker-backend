...
describe('BudgetController.getById', () => {
    beforeEach(() => {
        ...
        (Budget.findByPk as jest.Mock).mockImplementation((id) => {
            const updatedBudgets = budgets.filter(budget => budget.id === id)[0];
            return Promise.resolve(updatedBudgets);
        })
    });
...
describe('BudgetController.updateById', () => {
    it('Should update the budget and return a sucess message', async () => {
        const budgetMock = {
        ...
        const req = createRequest({
            ...
            url: '/api/v1/budget/:id',
            budget: budgetMock,
            body: { name: "Presupuesto actualizado", amount: 450000 }
        });
...
describe('BudgetController.deleteById', () => {
    it('Should delete the budget and return a sucess message', async () => {
        const budgetMock = {
